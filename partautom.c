...
static void buildTsection (uchar ** pattern, int np, int offs, int rows, int cols, int Rows, int Cols, Tsection T, mask Tin)
         ...
         */
{
    ...
    for (c = 0; c < Sigma; c++)
        if (~t[c]) {            /* the character appears in the pattern, process it */
            ...
            T[c] = Tin;         /* the char is not present, fast process */
         ...
         */
{
    ...
    if (M->I && M->J) {
        ...
        M->T.d1 = (Tsection *) malloc ((M->J + M->I + 1) * sizeof (Tsection));
        ...
        /* builds the D matrix for problem M (current and new D) */
{
    ...
    if (M->I && M->J) {
        ...
        for (j = 0; j <= M->J + 1; j++)
            M->D.d2[j][-1] = M->newD.d2[j][-1] = M->Din;
        for (i = 0; i <= M->I; i++) {
    ...
    if (M->J && (M->lc > 1)) {
        ...
        for (j = 0; j <= M->J; j++) {
            ...
            while (++r < M->lc)
                M->Gany.d1[j] |= M->Gany.d1[j] << (M->lr + 1);
        }
    }
    else {
        r = del;
...
static double estimate (int m, int k, int c, int r, bool active, bool vert)
{
    ...
    if (vert) {
        ...
        if (I > 1)
            if (J > 1)
                cost = 12 + (active ? 166 : 0) + 227 * I * (active ? actdiag : J);
            else;
        else if (lc > 1) {
            if (active) {       /* compute prob of activating last diag of fst
            ...
            cost = 113 + (active ? 20 : 0) + 83 * (active ? actdiag : J);
    }
    return cost;
}
...
double estimatePartAutom (int m, int k, int c, int np)
{
    ...
    if (2 * (m - k) <= W)
        vcost = estimate (m, k, c, np, false, true);

    if (pautomForceActive)
        active = true;
    else if (pautomForceNoActive)
        active = false;
    else
        active = (acost < nacost);
    ...
    if ((2 * (m - k) <= W) && (pautomForceVert || (!pautomForceHoriz && (vcost < cost))))
        return vcost;
...
void prepSPartAutom (PartAutom * M, uchar ** pattern, int np, int k, int c, bool V)
{
    ...
    /* we may prefer a vertical partition  */
    if ((2 * (m - k) <= W) && (pautomForceVert || (!pautomForceHoriz && (vcost < cost)))) {     /* vertical partition preferred */
        int lr = W / (m - k) - 1;
        int I = (k + lr) / lr;
    ...
    if (useSForce)
        useS = true;
    else if (dontuseSForce)
        useS = false;
    else
        useS = (k / (double) m < log (c) / (3 * log (m)));
