	...
	if (M->I)
	     if (M->lc > 1)
		    ...
		    (!active ? searchXPartAutomHV1c :
		     (useS ? searchFPartAutomHV1c : searchPartAutomHV1c)), puts("1");
	else if (M->lc > 1)
                  M->search = 
		     V ? searchVPartAutomH : 
		    (!active ? searchXPartAutomH :
		     (useS ? searchFPartAutomH : searchPartAutomH)), puts("2");
             else M->search = 
		     V ? searchVPartAutomH1c : 
		    (!active ? searchXPartAutomH1c :
		     (useS ? searchFPartAutomH1c : searchPartAutomH1c)), puts("3");
     else 
	if (M->I)
	     M->search = 
		     V ? searchVPartAutomV : 
		     (useS ? searchFPartAutomV : searchFPartAutomV), puts("4");
	else M->search = 
			V ? searchVAutom : 
